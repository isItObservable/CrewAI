...
* **CrewAI Skills + the built-in tool library** — scaffold with `npx skills add crewaiinc/skills`, reach for `crewai-tools` (40+ ready-made tools) before writing your own, publish custom tools to PyPI with `uv build` / `uv publish`
...
* **CopilotKit** (OSS, self-hosted) as the browser UI that drives the crew's `/kickoff` API via the AG-UI protocol — never CopilotKit Cloud/AMP

> **Provenance (2026-09-09):** tutorial aligned with the v3 storyboard per board asks [ISI-4078] and [ISI-4095] (CopilotKit as the tutorial UI).
...
> **Note — no monolithic `deployment.sh` here.** Unlike our infrastructure episodes, the CrewAI tutorial is **host-first**: you build and run the crew on your laptop (Steps 1–6), *then* containerize and deploy it (Steps 7–11). Each stage is a copy-pasteable command below, so you can stop after the local run if all you want is to understand CrewAI.
...
> Prefer to see how CrewAI scaffolds a project? **Skills first**: `npx skills add crewaiinc/skills` pulls CrewAI's official registry into your coding agent (build-time scaffolding — it doesn't run inside the crew), and `crewai create crew demo` produces the same `config/agents.yaml` + `config/tasks.yaml` + `crew.py` shape this repo uses. We ship a ready-made crew so you can go straight to running it.
...
Leave `OTEL_EXPORTER_OTLP_ENDPOINT` unset to run without telemetry (no crash). To silence CrewAI's **native telemetry** (anonymous product analytics on a private, isolated TracerProvider that is never registered global and never exported to your backend — it doesn't collide with OpenLIT and doesn't replace the `gen_ai.*` layer), set `CREWAI_DISABLE_TELEMETRY=true` (the crew does this by default). **Don't use `OTEL_SDK_DISABLED=true` for that** — it kills your own OpenLIT pipeline too.
...
### Step 9: Drive the crew from the browser — self-hosted CopilotKit UI

**Goal:** Replace curl with a chat panel: type a brief, watch the crew answer. Self-hosted OSS CopilotKit over the AG-UI protocol — no CopilotKit Cloud, no extra model key.

```bash
# 1. crew service (local or port-forwarded)
kubectl -n bmad-crew port-forward svc/bmad-crew 8000:80 &
# 2. backend-for-frontend (AG-UI over SSE → POST /kickoff)
cd bmad-crew && KICKOFF_URL=http://localhost:8000/kickoff uv run bmad-crew-bff
# 3. the UI
cd ../frontend && npm install && npm run dev     # http://localhost:5173
```

**Verify:** type `project: Status page` + a brief — a status note streams back, then the QA report lands inline when the crew finishes. Full walkthrough (including the echo-mode smoke test with no LLM): **[TUTORIAL.md §11](./TUTORIAL.md)** and [`frontend/README.md`](./frontend/README.md).
